...
// Function to fetch favorites and wishlist
export const fetchFavoritesAndWishlist = async (token) => {
  ...
  export const addMovieToFavorites = async (token, movieId) => {
    const response = await axios.post('http://localhost:5000/api/profile/favorites', { movieId }, {
        headers: {
            'Authorization': `Bearer ${token}`,
        },
    });
...
export const addMovieToWishlist = async (token, movieId) => {
    const response = await axios.post('http://localhost:5000/api/profile/wishlist', { movieId }, {
        headers: {
            'Authorization': `Bearer ${token}`,
        },
    });
